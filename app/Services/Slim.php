...
class Slim {
    ...
    public static function saveImage($input, $path)
    {
        $img = Request::input($input);
        $imageInfo = json_decode($img);
        $imageExtension = explode('.', $imageInfo->file);
        $extension = end($imageExtension);
        
        if(Request::input('name')){
            $img_name = Request::input('image_name') . '.' . $extension;
            $name = Request::input('image_name');
        }
        else{
            $img_name = time() . rand(1,100) . '.' . $extension;
            $name = time() . rand(1,100);
        }

        copy($imageInfo->path, base_path() . '/public/' . $path . '/' . $img_name);
        Image::make($path . '/' . $img_name)->encode('jpg')->save($path . '/' . $name . '.jpg');
